...
const paymentFormSchema = z.object({
  amount: z.string().min(1, 'Valor é obrigatório').transform((val) => Number.parseFloat(val.replace(',', '.'))),
  ...
  expiresIn: z.string().optional().transform((val) => val ? Number.parseInt(val) : undefined),
  ...
  const onSubmit = async (data: PaymentFormData) => {
    ...
    try {
      ...
      toast.success(`🎉 Cobrança PIX criada com sucesso!`, {
        description: `Valor: R$ ${data.amount.toFixed(2)} via ${data.pixProvider === 'inter' ? 'Banco Inter' : '4send'}`
      });
