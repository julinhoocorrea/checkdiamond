...
Este é um email automático, por favor não responda.
  ...
  /**
   * Simula envio de email (em produção, integraria com serviço real)
   */
  async sendEmail(template: EmailTemplate): Promise<{ success: boolean; message: string; emailPreview?: string }> {
    try {
      ...
      return {
        success: true,
        message: `Email enviado com sucesso para ${template.to}`,
        emailPreview: template.html // Para demonstração
      };
